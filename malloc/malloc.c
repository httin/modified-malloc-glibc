...
void*  __tin_malloc(size_t, const char*, int);
libc_hidden_proto (__tin_malloc)
...
/* tinht */
void __dump_inuse_chunk (FILE *fp);
void __tinht25 (void);
...
/*------------------------ Public wrappers. --------------------------------*/
void *
__tin_malloc (size_t bytes, const char* file, int line)
{
  mstate ar_ptr;
  void *victim;
  void *v_victim;
  ...
  v_victim = victim + musable(victim) - 32;

  for (i = 0; i < 32; i++) {
    *((char *)(v_victim + i)) = buf[i];
  }

  return victim;
}
libc_hidden_def (__tin_malloc)
...
int_mallinfo (mstate av, struct mallinfo *m)
{
  ...
  m->smblks += nfastblocks; /* number of free fastbins */
  m->ordblks += nblocks; /* number of free-chunks */
  m->fordblks += avail; /* total free mem */
  m->uordblks += av->system_mem - avail; /* total allocated mem */
  m->arena += av->system_mem; /* system mem of arena */
  m->fsmblks += fastavail; /* available mem in freed fastbins */
  if (av == &main_arena)
    {
      m->hblks = mp_.n_mmaps;
...
/********************* user's APIs **********************/
static void list_free_chunk(mstate av)
{
  size_t i;
  mbinptr b;
  mchunkptr p;
  ...
  /* top-most chunk */
  fprintf(stderr, "top-most-chunk addr: %p, size: %lu\n", 
        av->top, chunksize(av->top));

  /* last-remainder chunk */
  fprintf(stderr, "last-remainder-chunk addr: %p\n", 
        av->last_remainder);

  /* traverse fastbins array */
  fprintf(stderr, "traversing fastbins...\n");
  for (i = 0; i < NFASTBINS; ++i)
    {
      for (p = fastbin (av, i); p != 0; p = p->fd)
        {
          fprintf(stderr, "%lu\taddr: %p, size: %lu\n", i, p, chunksize(p));
        }
    }

  fprintf(stderr, "traversing bins...\n");
  /* traverse regular bins */
  for (i = 1; i < NBINS; ++i)
    {
      b = bin_at (av, i);
...
void __dump_inuse_chunk (FILE *fp)
{
  ...
  for (i = 0, ar_ptr = &main_arena;; i++)
  {
     ...
     * then traversing every heap belonging to that arena
     */
    if (i > 0) 
    {
      ...
      {
        j = 0;
        fprintf (fp, "\t[Heap %d] %p, size %lx\n", j++, heap, (long)heap->size);
        ptr = (heap->ar_ptr != (mstate) (heap + 1)) ?
              (char *) (heap + 1) : (char *) (heap + 1) + sizeof (struct malloc_state);
        p = (mchunkptr) (((unsigned long) ptr + MALLOC_ALIGN_MASK) &
                         ~MALLOC_ALIGN_MASK);
  
        /* with every specific 'heap', we check all chunks from 
         *'bottom' to the 'top', and dump information of in-use chunk
         */
        for (;; p = next_chunk (p))
          {
      ...
      } while ( (heap = heap->prev) != NULL);
    } 
    else /* This is for main_arena */
    {
      p = (mchunkptr)(((char*)top(ar_ptr) + chunksize(top(ar_ptr))) - ar_ptr->system_mem);
      for (;;)
        {
          if (p == top (ar_ptr))
            {
...
void __tinht25 (void)
{
  ...
  printf(
    ...
    "\tnumber of mmapped regions: %d (main arena)\n"
    "\tspace in mmapped regions: %d (main arena)\n"
    "\talways 0, preserved for backwards compatibility: %d\n"
    "\ttotal allocated space: %d\n"
    "\ttotal free space: %d\n"
    "\ttop-most, releasable space (via malloc_trim): %d (main arena)\n",
    m.arena, m.ordblks, m.smblks, m.fsmblks, m.hblks, 
    m.hblkhd, m.usmblks, m.uordblks, m.fordblks, m.keepcost
  );
}
...
/*----------------------------- __tin_malloc -----------------------------*/
strong_alias (__tin_malloc, __malloc_t) strong_alias (__tin_malloc, malloc_t)
/*----------------------------- __tin_malloc -----------------------------*/
weak_alias (__tinht25, tinht25)
weak_alias (__dump_inuse_chunk, dump_inuse_chunk)
/* ------------------------------------------------------------
   History:
